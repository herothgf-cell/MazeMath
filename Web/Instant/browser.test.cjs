...
const craftTool=async(i)=>{await page.locator('#recipe'+i).click();const recipe=await page.evaluate(i=>MM.recipes[i],i);for(let cell=0;cell<9;cell++){const mat=recipe[cell];if(mat<0)continue;await page.locator('#pick'+mat).click();await page.locator('#cell'+cell).click();}assert.match(await page.locator('#craftResult').innerText(),new RegExp(['곡괭이 팔','파워 렌치','점프 부스터','에너지 실드','탐험 센서'][i]));assert.equal(await page.locator('#makeCraft').isEnabled(),true);await page.locator('#makeCraft').click();await page.waitForTimeout(80);};
...
await place(14,0);await use();if(await page.locator('#tutorialOk').count())await page.locator('#tutorialOk').click();
await place(18,0);await use();await craftTool(2);assert.ok(await page.evaluate(()=>MMApp.state().owned[2]));await page.locator('#close').click();
...
await place(14,0);await use();if(await page.locator('#tutorialOk').count())await page.locator('#tutorialOk').click();
await place(18,0);await use();await craftTool(4);await craftTool(3);assert.ok(await page.evaluate(()=>MMApp.state().owned[4]&&MMApp.state().owned[3]));await page.locator('#close').click();
