...
function title(){let old=load();open('title','',`<div class="center"><span class="tag">MAZEMATH · MOBILE PLAY</span><div class="hero"><div class="face"></div><div class="glass"></div></div><h1>모모의<br>블록 탐험</h1><p>길을 찾고, 숫자 장치를 풀고,<br>직접 만든 도구로 모모를 도와주세요.</p><span class="tag">3층 미로 · 수학 · 제작 · 골렘</span></div><div class="stack">${old?'<button class="btn primary wide" id="continue">이어하기</button><button class="btn wide" id="chaptersTitle">챕터 선택</button>':''}<button class="btn ${old?'':'primary'} wide" id="new">새 탐험 시작</button></div><p class="sub center">별도 설치 없이 터치로 플레이합니다.<br>가로 화면을 권장하지만 세로 화면도 지원합니다.<br><b>모바일 즉시 플레이판</b> · Unity 본편과 별도 버전/저장</p>`);bind('new',()=>{if(old){open('confirm','새 탐험','<p>모바일 웹판의 이어하기 기록을 새로 시작할까요?</p><button class="btn primary wide" id="yes">새로 시작</button>');bind('yes',newRun);}else newRun();});bind('continue',()=>{install(old);running=true;hide();toast('모모: 돌아오셨네요! 탐험을 이어가요.');});bind('chaptersTitle',()=>{install(old);running=false;chapterSelect();});}
